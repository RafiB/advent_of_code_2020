...
PUZZLE_INPUT = """rtdhb jxspzl bjgjgb tjtgbf jdmsljx psjq jqktb qjtlsg trlx vpzxk rfccb lccbpx nrz qndq vnml cvzqq bdt sqqr tpfj rczt xdpvd mpms npk tgpv tvcrkml tclx bkgmcsx fbvsh rjdqt hdcj rjjp lggrq vxnd xhdf tgkgzqj dgxrzglk grqfjk kgckc rthjg hbnf clnkzp zqtd vndnsd jspkl rbxklnd cdvjskn vblsdx lxxvb chkqhd vdnnrpc vzqmj (contains wheat)
...
def solve(ingredients_list):
    allergens = {}
    all_ingredients = defaultdict(int)

    for line in ingredients_list.split("\n"):
        ingredient_list, allergen_list = line.split(" (")

        ingredients = ingredient_list.split(" ")
        known_allergens = allergen_list[len("contains "):-1].split(", ")

        for ingredient in ingredients:
            all_ingredients[ingredient] += 1

        for known_allergen in known_allergens:
            if known_allergen not in allergens:
                allergens[known_allergen] = set(ingredients)
            else:
...
def try_identify_allergens(allergens):
    identified = {k: set(v) for k, v in allergens.items()}

    for a in identified:
        possible_ingredients = identified[a]
        if len(possible_ingredients) == 1:
            ingredient = next(iter(possible_ingredients))
            for other_a in identified:
                if a == other_a:
                    continue
                identified[other_a].discard(ingredient)
